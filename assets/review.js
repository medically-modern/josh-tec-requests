...
function detailHtml(data) {
  ...
  right += detailSection('Activity', '<ul class="tl">' +
    (data.activity || []).map((a) => {
      const cls = a.action === 'created' ? 'tl-created' : a.action.startsWith('email') ? 'tl-email' : 'tl-status';
      return `<li class="${cls}">
        <div class="tl-text"><strong>${esc(ACT_LABEL[a.action] || a.action)}</strong>${a.detail ? ` — ${esc(a.detail)}` : ''}</div>
...
async function toggleDetail(row) {
  ...
  tr.innerHTML = `<td colspan="9"><div class="muted small"><span class="spinner dark"></span> Loading ${esc(ticket)}&hellip;</div></td>`;
  row.after(tr);
